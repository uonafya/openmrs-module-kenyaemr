...
public class ViralSuppressionCohortLibrary {
    public CohortDefinition suppressed(){
        ...
        String sqlQuery = "select a.patient_id as patient_id\n" +
                "from(select t.patient_id,vl.vl_date,vl.vl_result,vl.urgency,location_id from (\n" +
                ...
                "       de.date_started, fup.location_id\n" +
                ...
                "             )vl\n" +
                "    on t.patient_id = vl.patient_id  where (vl_result < 1000 or vl_result='LDL')) a where location_id = :defaultLocation;";
    ...
    public  CohortDefinition unsuppressed() {
        SqlCohortDefinition cd = new SqlCohortDefinition();
        String sqlQuery="select a.patient_id as patient_id\n" +
                "from(select t.patient_id,vl.vl_date,vl.lab_test,vl.vl_result,vl.urgency,location_id from (\n" +
                "select fup.visit_date,fup.patient_id, max(e.visit_date) as enroll_date,\n" +
                "       greatest(max(e.visit_date), ifnull(max(date(e.transfer_in_date)),'0000-00-00')) as latest_enrolment_date,\n" +
                "       greatest(max(fup.visit_date), ifnull(max(d.visit_date),'0000-00-00')) as latest_vis_date,\n" +
                "       greatest(mid(max(concat(fup.visit_date,fup.next_appointment_date)),11), ifnull(max(d.visit_date),'0000-00-00')) as latest_tca,\n" +
                ...
                "date_sub(:endDate , interval 12 MONTH) and date(:endDate)\n" +
                "             )vl\n" +
                "    on t.patient_id = vl.patient_id where vl_result >= 1000) a where a.location_id = :defaultLocation ;" ;
    ...
    public  CohortDefinition noCurrentVLResults() {
        String sqlQuery="select a.patient_id as patient_id\n" +
                ...
                "from kenyaemr_etl.etl_patient_hiv_followup fup\n" +
                "join kenyaemr_etl.etl_patient_demographics p on p.patient_id=fup.patient_id\n" +
                "join kenyaemr_etl.etl_hiv_enrollment e on fup.patient_id=e.patient_id and fup.location_id = e.location_id \n" +
                "left outer join kenyaemr_etl.etl_drug_event de on e.patient_id = de.patient_id and de.program='HIV' and date(date_started) <= date(:endDate)\n" +
                "left outer JOIN\n" +
                "(select patient_id, coalesce(date(effective_discontinuation_date),visit_date) visit_date,max(date(effective_discontinuation_date)) as effective_disc_date from kenyaemr_etl.etl_patient_program_discontinuation\n" +
                "where date(visit_date) <= date(:endDate) and program_name='HIV'\n" +
                ...
                ")vl\n" +
                "on t.patient_id = vl.patient_id) a where a.location_id = :defaultLocation;" ;
        SqlCohortDefinition cd = new SqlCohortDefinition();
    ...
    public  CohortDefinition noVLResults() {
        String sqlQuery="select t.patient_id from (\n" +
                ...
                "             p.DOB as date_of_birth, fup.location_id \n" +
                "            from kenyaemr_etl.etl_patient_hiv_followup fup\n" +
                "            join kenyaemr_etl.etl_patient_demographics p on p.patient_id=fup.patient_id\n" +
                "            join kenyaemr_etl.etl_hiv_enrollment e on fup.patient_id=e.patient_id and e.location_id = fup.location_id \n" +
                "            left outer join kenyaemr_etl.etl_drug_event de on e.patient_id = de.patient_id and de.program='HIV' and date(date_started) <= date(:endDate)\n" +
                ...
                "            ) t\n" +
                "            left join (\n" +
                "                select patient_id from kenyaemr_etl.etl_laboratory_extract where lab_test in (1305,856)\n" +
                "                group by patient_id\n" +
                "                )vl\n" +
                "            on t.patient_id = vl.patient_id\n" +
                "                where vl.patient_id is null and location_id = :defaultLocation ;";
        SqlCohortDefinition cd = new SqlCohortDefinition();
